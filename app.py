...
if page == 'Dashboard':
    ...
    hero('Model Performance', f'Understand how the {selected_model} model performs on unseen chronological test data.')
...
### How it works
The final prediction model is **{metadata.get('selected_model', 'Gradient Boosting')}**. Five regression algorithms were compared using the same chronological training and test data: Random Forest, Gradient Boosting, XGBoost, Decision Tree and Linear Regression. The final model was selected from their evaluation results.

Inputs include the route, weather condition, day of the week, hour, minute, month, day of month, weekend indicator and cyclical time features. The dataset is split chronologically: the first 80% is used for training and the final 20% for testing.
